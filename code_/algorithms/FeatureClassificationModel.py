...
class FeatureClassificationModel(Algorithm):
    ...
    def process_batch(self, batch, do_train=True):
        ...
        # ********************************************************
        start = time.time()
        #   print("DEBUG: before out featkey in FCM")
        out_feat_keys = self.out_feat_keys
        #  print("DEBUG: optimizer is in FeatureClassificationModel",self.optimizers)
        finetune_feat_extractor = self.optimizers['feat_extractor'] is not None
        # print("DEBUG: Finetune_feat_extractor is ",finetune_feat_extractor)
        ...
        if not finetune_feat_extractor:
            if isinstance(feat_var, (list, tuple)):
                ...
                #   print("DEBUG: before Variable operation in Feature")
                # 这里应该是volatile=True的情况
                with torch.no_grad():
                    dataX_var = Variable(dataX)
        #     print("DEBUG: after Variable operation in Feature")

        pred_var = self.networks['classifier'](feat_var)
        #   print("DEBUG: after all the bp in Feature")
        # ********************************************************
        ...
        if isinstance(pred_var, (list, tuple)):
            ...
            # record['prec2_c'+str(1+i)] = accuracy(pred_var[i].data, labels, topk=(2,))[0].item()
        #     print("DEBUG: pred i is: ",pred_var[i].data)
        else:
            loss_total = self.criterions['loss'](pred_var, labels_var)
            record['prec1'] = accuracy(pred_var.data, labels, topk=(1,))[1].item()
            # record['prec5'] = accuracy(pred_var.data, labels, topk=(5,))[0].item()
            record['prec_c1'] = accuracy(pred_var.data, labels, topk=(1,))[0][0]
            record['prec_c2'] = accuracy(pred_var.data, labels, topk=(1,))[0][1]
            record['prec_c3'] = accuracy(pred_var.data, labels, topk=(1,))[0][2]
            record['prec_c4'] = accuracy(pred_var.data, labels, topk=(1,))[0][3]
