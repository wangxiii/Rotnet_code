...
class NetworkInNetwork(nn.Module):
    def __init__(self, opt):
        ...
        nChannels3 = int(96)
        num_inchannels=int(num_inchannels)
        nChannels=int(nChannels)
        blocks = [nn.Sequential() for i in range(num_stages)]
        # 1st block
        print("DEBUG: input num and nChannels ",num_inchannels,nChannels)
    ...
    def forward(self, x, out_feat_keys=None):
        ...
        feat = x
        """
        print("DEBUG Before extraction")
        feat1=self._feature_blocks[0](feat)
        print("DEBUG After extraction")
        print("DEBUG: Feat's shape", feat.shape)
        """
        for f in range(max_out_feat+1):
          #  print("DEBUG: In Loop")
           # print("DEBUG:What is f:",)
            #这里出问题了
            feat = self._feature_blocks[f](feat)
          #  print("DEBUG: After feat")
            key = self.all_feat_names[f]
          #  print("DEBUG: before if")
            if key in out_feat_keys:
             #   print("DEBUG: into if")
                out_feats[out_feat_keys.index(key)] = feat
              #  print("DEBUG: In Loop if")
        print("DEBUG:  Features out")
