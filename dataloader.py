...
_IMAGENET_DATASET_DIR = '/home/vision/data'
_PLACES205_DATASET_DIR = './data_example/cars_ori'
_CELEBA_DATASET_DIR='./data_example/Nimstim_hsf_color_64'
_Face_DATASET_DIR='/home/vision/data'
...
class GenericDataset(data.Dataset):
    def __init__(self, dataset_name, split, random_sized_crop=False,
        ...
        if self.dataset_name=='imagenet':
            ...
            if(self.split =='train'):
                split_data_dir = _IMAGENET_DATASET_DIR + '/ILSVRC2012_img_train'
            else:
                split_data_dir = _IMAGENET_DATASET_DIR + '/ILSVRC2012_img_val'
            ...
            self.transform = transforms.Compose(transforms_list)
            if (self.split == 'train'):
                split_data_dir = _IMAGENET_DATASET_DIR + '/vggface_train'
            else:
                split_data_dir = _IMAGENET_DATASET_DIR + '/ILSVRC2012_img_val'
            self.data = datasets.ImageFolder(split_data_dir, self.transform)
        elif self.dataset_name=='celebrityh':
            #不知道这里如何定义
        ...
        #这个参数是控制什么的？只在cifar10中使用，挑选前k个作为训练/测试集
...
class DataLoader(object):
    def __init__(self,
        ...
        ##What's meaning of this epoch_size???? maybe just mark
    ...
    def get_iterator(self, epoch=0):
        ...
        if self.unsupervised:
            ...
            # ind
            # ex of an image it returns the 4 rotated copies of the image
            # plus the label of the rotation, i.e., 0 for 0 degrees rotation,
            # 1 for 90 degrees, 2 for 180 degrees, and 3 for 270 degrees.这里设置标签了
            def _load_function(idx):
                idx = idx % len(self.dataset)
                img0, _ = self.dataset[idx]
             #   img0=img0.copy()
                #这里可能出现的张量为负数的问题，加copy()是否能够解决
              #  img1=self.transform(img0).copy()
                rotated_imgs = [
                    self.transform(img0),
